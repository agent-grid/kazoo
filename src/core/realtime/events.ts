...
/** The supervisor model invoked one of its two tools (SUPERVISOR_SPEC §2a/§2d).
 *
 *  Emitted when a `response.function_call_arguments.done` server event lands.
 *  The orchestrator (loop.ts) routes on this:
 *    - `delegate_to_executor` → `executor.submit` (read-only wrapper when
 *      `reason === 'unknown_fact'`, task-mode for `'new_task'`).
 *    - `stop_executor`        → `executor.cancelTurn({ dropQueue })`.
 *
 *  `callId` is echoed back to the model via `session.sendToolResult` so it can
 *  voice its own brief ack/confirm. `args` is the parsed tool arguments; the
 *  shape depends on `name`. */
export type ToolCallEvent = {
  type: 'tool-call'
  name: 'delegate_to_executor' | 'stop_executor'
  callId: string
  args:
    | { task?: string; reason?: 'new_task' | 'unknown_fact' } // delegate_to_executor
    | { drop_queue?: boolean } // stop_executor
}
