...
// The supervisor's two tools (SUPERVISOR_SPEC §2d). The Realtime model decides
// answer-vs-delegate-vs-stop via `tool_choice: 'auto'` + these definitions +
// the supervisor instructions (persona.ts). The orchestrator stays mechanical:
// it forwards the resulting tool call and never classifies intent itself.
//
// `reason` on delegate_to_executor is LOAD-BEARING: `unknown_fact` selects the
// read-only delegation path in loop.ts (the orchestrator wraps the task with a
// no-mutation instruction), so a delegated *question* can never let the worker
// edit files. `new_task` submits unwrapped (edits allowed).
const SUPERVISOR_TOOLS = [
  {
    type: 'function',
    name: 'delegate_to_executor',
    description:
      'Hand work to the coding agent (your own hands). Call this for ANY new ' +
      'coding task the user asks for, AND for ANY question you cannot answer ' +
      'truthfully from this conversation — i.e. whenever you would otherwise ' +
      'have to guess a fact about the code, files, or project. Prefer calling ' +
      'this over guessing. The coding agent runs continuously and is not ' +
      'interrupted; you narrate its progress as it works.',
    parameters: {
      type: 'object',
      properties: {
        task: {
          type: 'string',
          description:
            'The task or question to hand off, phrased in full as an ' +
            "instruction to the coding agent. Include the user's intent.",
        },
        reason: {
          type: 'string',
          enum: ['new_task', 'unknown_fact'],
          description:
            "'new_task' for work the user requested (the agent may edit files). " +
            "'unknown_fact' when you are ONLY finding out something you don't " +
            'know — the agent must answer read-only and change nothing.',
        },
      },
  ...
  },
  {
    ...
    name: 'stop_executor',
    description:
      "Stop the coding agent's current work. Call ONLY when the user explicitly " +
      "tells you to stop, cancel, halt, or drop what you're doing. Set " +
      'drop_queue true if they want all pending work cleared too, not just the ' +
      'current step.',
    parameters: {
      type: 'object',
      properties: {
        drop_queue: {
          type: 'boolean',
          description: "True for 'drop everything' / 'cancel all'.",
        },
      },
      required: [],
    },
...
type ServerEvent = {
  ...
  // Function-call streaming (SUPERVISOR_SPEC §2a). The
  // `response.function_call_arguments.done` event carries the tool name, the
  // server-assigned call id, and the FULL arguments JSON as a string. Without
  // these the events hit `default` and the supervisor's routing is silently
  // dropped.
  call_id?: string
  name?: string
  arguments?: string
...
export class RealtimeSession {
  ...
  /** Return a function-call's output to the model and let it voice its own
   *  brief ack/confirm (SUPERVISOR_SPEC §2e).
   *
   *  Called by the orchestrator immediately after it routes a `tool-call`
   *  event — fire-and-forget, never waiting on the executor. Two-step:
   *    1. `conversation.item.create` with a `function_call_output` item closes
   *       the call the model opened (matched by `callId`).
   *    2. `response.create` (audio) lets the model speak its own short
   *       acknowledgement ("On it" / "Let me check that" / "Stopped") in its
   *       own voice — we no longer hardcode the ack text.
   *
   *  No-op while a response is already in flight: the `responseActive` guard is
   *  exactly the serialization mechanism that keeps acks, answers, and
   *  narration taking turns through the single response slot. The function
   *  output itself is sent regardless so the call is always closed; only the
   *  follow-up `response.create` is gated. */
  sendToolResult(callId: string, payload: unknown): void {
    if (this.state !== 'active') return
    if (!callId) return
    this.send({
      type: 'conversation.item.create',
  ...
  /** Inject a CLEARLY-MARKED, non-assistant awareness record SILENTLY — no
   *  `response.create`, so it is never voiced (SUPERVISOR_SPEC §4b).
   *
   *  This is how the orchestrator's `[WORK-LOG]` reaches the model's context so
   *  the voice can answer "what did you change / what are you doing / is it
   *  done" truthfully without delegating. Critically it is injected as a
   *  `role: 'user'` `input_text` item — a RECORD, not the model's own speech.
   *  Injecting awareness as a first-person assistant turn would be hallucination
   *  fuel: the model could fluently EXTEND its own apparent sentence with no
   *  tool call. As a marked external record, the persona tells it to quote but
   *  never extrapolate.
   *
   *  Unlike `injectNarration` there is no `response.create` here — the item just
   *  sits in the conversation as background context. Safe to call while a
   *  response is in flight (it doesn't contend for the response slot). */
  injectAwareness(text: string): void {
    if (this.state !== 'active') return
    if (!text) return
    this.send({
      type: 'conversation.item.create',
  ...
  private sendSessionUpdate(): void {
    this.send({
      ...
      session: {
        ...
        // directly (no tool) when it can do so truthfully from context.
        ...
        audio: {
          input: {
            ...
            turn_detection: {
              ...
              // SUPERVISOR MODE (SUPERVISOR_SPEC §2b). The voice is now a
              // supervisor, not a mute narrator: it answers the user from
              // context and decides delegate-vs-answer-vs-stop via its two
              // tools. So we let server-VAD fire exactly one response per user
              // turn.
              //
              //   create_response: true      — voice responds each user turn
              //                                (FLIPPED from false; the old
              //                                narrator-only gag is gone — the
              //                                anti-fabrication rule in the
              //                                persona replaces it)
              //   interrupt_response: true   — user speech cancels the single
              //                                in-flight VOICE response (answer,
              //                                ack, OR narration — they all
              //                                share one default-conversation
              //                                slot and serialize through it)
              //
              // Narration still flows through the SAME slot via
              // `injectNarration` + the `responseActive` guard, so answers and
              // narration take turns rather than overlapping.
              //
  ...
  private handleServerEvent(ev: ServerEvent): void {
    switch (ev.type) {
      ...
      // Function-call streaming for the supervisor's two tools
      // (SUPERVISOR_SPEC §2e). The arguments arrive WHOLE on `.done`, so the
      // `.delta` event is buffered/ignored.
      case 'response.function_call_arguments.delta':
        return

      case 'response.function_call_arguments.done': {
        const name = ev.name
        if (name !== 'delegate_to_executor' && name !== 'stop_executor') {
          this.logger.warn({ name }, 'realtime: unknown function call name, ignoring')
          return
        }
        ...
        try {
          ...
          this.logger.warn(
            { err: err instanceof Error ? err.message : String(err), raw: ev.arguments },
            'realtime: function_call_arguments parse fail',
          )
          args = {}
        }
        // Emit the tool-call onto the RealtimeEvent bus. The orchestrator
        // (loop.ts) acts on it (submit / cancelTurn) and then calls
        // `sendToolResult(callId, …)` to let the model voice its own ack.
        this.emit({
          type: 'tool-call',
          name,
          callId: ev.call_id ?? '',
