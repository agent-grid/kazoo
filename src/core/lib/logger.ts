// Pino → ND-JSON file. This logger runs in the Electron MAIN process only;
// the renderer must NOT import it (no pino in the browser) — it ships log
// lines to main over the bus `log` event instead. In main, stdout is free
// (Ink no longer owns it), so a dev console transport can be added without
// trashing any UI. Every module receives a logger via dependency injection
// so tests can swap in a no-op or in-memory recorder.
//
// Redaction (security-review): we deliberately log a lot at debug — bash
// commands the model wanted to run, tool inputs, narration text. Some of
// that can carry secrets if the model is tricked into echoing them, OR if a
// tool result happens to contain credentials. Pino `redact` masks the
// high-risk paths even in debug mode. Add to the list rather than removing —
// silence by default is the safer bias.
