...
export class AsyncQueue<T> {
  ...
  /** Drop all queued-but-not-yet-consumed items WITHOUT closing the queue.
   *  Used by the executor's `cancelTurn({ dropQueue: true })` to drain pending
   *  user turns (SUPERVISOR_SPEC §3c.3) — "drop everything" / "cancel all".
   *  Returns the number of items dropped. Does not touch waiters: a consumer
   *  parked on `next()` keeps waiting for the next pushed item. */
  clear(): number {
